...
while read NAME; do
	...
	if [ ! -f "$OUTDIR/4_Tumor_Host_Filtering/final_regenotyped_"${NAME%.*}"_genotyped.multiallelic.vcf" ]; then
		
		echo "$NAME does not exist"
		
		# Identify the filtered VCFs for matching tumor(s) and Host
				ls -1 $OUTDIR/3_SNV_filtering_individual/platypusVariants_${NAME%.*}[A-Z]*.indels.vcf > $OUTDIR/4_Tumor_Host_Filtering/${NAME%.*}.temp.vcf.list
				#ls -1 $OUTDIR/3_SNV_filtering_individual/platypusVariants_100H-Devil_default.multiallelic.snvs.filtered.indels.vcf >> $OUTDIR/4_Tumor_Host_Filtering/${NAME%.*}.temp.vcf.list
			
		# Identify the Appropriate BAM Files that will be used for Platypus
				ls -1 $BAMSDIR/${NAME%.*}[A-Z]*.bam > $OUTDIR/4_Tumor_Host_Filtering/${NAME%.*}.bamlist.txt
				#ls -1 $BAMSDIR/100H*.bam >> $OUTDIR/4_Tumor_Host_Filtering/${NAME%.*}.bamlist.txt


		# Merge the Tumor(s) and Host Filtered SNV VCFs together
				while read FILE; do 
			
					egrep -v "^#" $FILE |cut -f1-5 >> $OUTDIR/4_Tumor_Host_Filtering/${NAME%.*}.temp.vcf
